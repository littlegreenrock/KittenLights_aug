...
// ########## WEIRD ALTERNATIVE, requires testing ###############
const byte TableG_RB[100][2]		{
		{14, 0 },		{19, 0 },		{23, 0 },		{28, 0 },
		{34, 0 },		{39, 0 },		{44, 0 },		{49, 0 },
		{54, 0 },		{60, 0 },		{65, 0 },		{70, 2 },
		{75, 4 },		{80, 7 },		{85, 11 },	{90, 15 },
		{95, 19 },	{100, 24 },	{105, 29 },	{110, 35 },
		{114, 40 },	{119, 46 },	{124, 51 },	{128, 57 },
		{133, 63 },	{137, 69 },	{142, 75 },	{146, 81 },
		{151, 87 },	{155, 93 },	{159, 99 },	{163, 105 },
		{168, 111 },	{172, 117 },	{176, 124 },	{180, 130 },
		{184, 136 },	{188, 142 },	{192, 147 },	{196, 153 },
		{199, 159 },	{203, 165 },	{207, 171 },	{211, 177 },
		{214, 183 },	{218, 188 },	{222, 194 },	{225, 200 },
		{229, 206 },	{232, 211 },	{236, 217 },	{239, 222 },
		{243, 228 },	{246, 233 },	{250, 239 },	{253, 244 },
		{255, 255 },	{241, 254 },	{232, 236 },	{224, 222 },
		{218, 212 },	{213, 203 },	{208, 196 },	{205, 189 },
		{201, 184 },	{198, 179 },	{195, 175 },	{193, 171 },
		{191, 167 },	{189, 164 },	{187, 161 },	{185, 158 },
		{183, 156 },	{182, 153 },	{180, 151 },	{179, 149 },
		{177, 147 },	{176, 145 },	{175, 143 },	{174, 142 },
		{172, 140 },	{171, 139 },	{170, 137 },	{169, 136 },
		{169, 135 },	{168, 133 },	{167, 132 },	{166, 131 },
		{165, 130 },	{164, 129 },	{164, 128 },	{163, 127 },
		{162, 126 },	{161, 125 },	{161, 124 },	{160, 123 },
		{159, 122 },	{159, 121 },	{158, 120 },	{158, 120 },
	};	//	200 B

byte K_to_idx(int K) {
	int i;
	byte Col;
	if (K>6700) i = ((K-6700) / 200) +57;
	else i = (K/100) - 10;
	i = constrain(i,0,100);
	return i;
}
byte red(int idx) {
	if (idx > 57)	return TableG_RB[idx][1];
	return 255;
}
byte green(int idx) {
	return TableG_RB[idx][0];
}
byte blue(int idx) {
	if (idx < 57)	return TableG_RB[idx][1];
	return 255;
...
public:
	int NEOPIX_WhiteLED_temp = 5000;		//	best to use: 0, 4000, 5000, 6000, 7000
...
// alternatively...if you want more float in your bloated float algo
	...
	void HellandSpread(Adafruit_NeoPixel& PixelName, const int LEDcount, int maxKelvin=15000, int minKelvin=1500) {
		maxKelvin = constrain(maxKelvin, 6000, 40000);
		minKelvin = constrain(minKelvin, 1000, 7000);
		int K_step = (maxKelvin-minKelvin)/(LEDcount-1);
		int _LedNo{0};
		for (int Kelvin = 1000; Kelvin <= maxKelvin; Kelvin+=K_step) {
			// optional serial feedback
			if (_LedNo==0) Serial.printf("from %5i K  -", Kelvin);
			else if (_LedNo==LEDcount-1) Serial.printf("to-  %5i K across %i LEDs\n", Kelvin, LEDcount);
			PixelName.setPixelColor(_LedNo++, this->Calculate(Kelvin));
			PixelName.show();
			if (_LedNo>LEDcount) break;		//	 you have run out of LEDs, and I wrote a bad loop
		}
	}
